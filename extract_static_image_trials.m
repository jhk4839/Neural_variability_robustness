...
%% Extract firing rate for specific stimulus types
for ises = 1:size(sessions, 1)
    if ismember(ises, find(bool_BO)) % Brain Observatory 1.1 sessions (natural scenes, static gratings, spontaneous)
    % if ~ismember(ises, find(bool_BO)) & ~ismember(ises, [49 50]) % Functional Connectivity sessions with V1 units (drifting gratings, spontaneous)
        tic
        ...
        % 2. static grating trials
        sgtrials = strcmp(stimtable.stimulus_name, 'static_gratings');
        sgtrialstart = stimtable.start_time(sgtrials);
        sgtrialori = stimtable.orientation(sgtrials); % orientation
        sgtrialsf = stimtable.spatial_frequency(sgtrials); % spatial frequency
        sgtrialph = str2double(stimtable.phase(sgtrials)); % phase

        % stimulus duration is 0.25s, iti is 0.25s.
        psthtlins = (-250:500)';
        psthtrialinds = floor(sgtrialstart'/Tres)+1 + psthtlins;

        % V1
        psthsg = false(length(psthtlins), nnz(sgtrials), numel(nwbunitsV1));
        for ii = 1:numel(nwbunitsV1)
            tempST = spiketrain(:,ii);
            psthsg(:,:,ii) = tempST(psthtrialinds);
        end
        clear tempST psthtrialinds

        % V1
        Rsg = (1/Tres)*squeeze(mean(psthsg(psthtlins>0&psthtlins<=250,:,:),1));
        Nunits = size(Rsg,2);
        Ntrials = size(Rsg,1);    
        save(strcat(pathsv, 'Rsg_V1_', sesid, '.mat'), ...
            'sgtrialstart', 'psthsg', 'Rsg', 'Nunits', 'Ntrials', 'sgtrialori', "sgtrialsf", "sgtrialph")

        % 3. drifting grating trials (Functional Connectivity 75 repeats)
        dg75trials = strcmp(stimtable.stimulus_name, 'drifting_gratings_75_repeats');
        dg75trialstart = stimtable.start_time(dg75trials);
        dg75trialori = stimtable.orientation(dg75trials); % orientation
        dg75trialct = stimtable.contrast(dg75trials); % contrast

        % stimulus duration is 2s, iti is 1s.
        psthtlins = (0:2000)';
        psthtrialinds = floor(dg75trialstart'/Tres)+1 + psthtlins;

        % V1
        psthdg75 = false(length(psthtlins), nnz(dg75trials), numel(nwbunitsV1));
        for ii = 1:numel(nwbunitsV1)
            tempST = spiketrain(:,ii);
            psthdg75(:,:,ii) = tempST(psthtrialinds);
        end
        clear tempST psthtrialinds

        % V1
        Rdg75 = (1/Tres)*squeeze(mean(psthdg75(psthtlins>0&psthtlins<=2000,:,:),1));
        Nunits = size(Rdg75,2);
        Ntrials = size(Rdg75,1);    
        save(strcat(pathsv, 'Rdg75_V1_', sesid, '.mat'), ...
            'dg75trialstart', 'psthdg75', 'Rdg75', 'Nunits', 'Ntrials', 'dg75trialori', 'dg75trialct')

        % 4. spontaneous block (Functional Connectivity 30 min block)
        spttrials = strcmp(stimtable.stimulus_name, 'spontaneous');
        spttrialstart = stimtable.start_time(spttrials);
        spttrialstop = stimtable.stop_time(spttrials);
        [~, longblk_ind] = max(spttrialstop - spttrialstart); % extract only the longest spontaneous block (30 min)

        spttrialstart = spttrialstart(longblk_ind);
        spttrialstop = spttrialstop(longblk_ind);
        longblk_dur = spttrialstop - spttrialstart; % seconds

        % one or more long blocks
        psthtlins = (0:floor(max(longblk_dur)/Tres))';
        psthtrialinds = floor(spttrialstart'/Tres)+1 + psthtlins;

        % V1
        psthspt = false(length(psthtlins), numel(longblk_ind), numel(nwbunitsV1));
        for ii = 1:numel(nwbunitsV1)
            tempST = spiketrain(:,ii);
            psthspt(:,:,ii) = tempST(psthtrialinds);
        end
        psthspt = squeeze(psthspt);
        clear tempST psthtrialinds

        % V1
        Nunits = size(psthspt,ndims(psthspt)); % last dimension
        save(strcat(pathsv, 'Rspt_V1_', sesid, '.mat'), ...
            'spttrialstart', 'psthspt', 'Nunits') % functional connectivity

        % 5. spontaneous block (Brain Observatory 1.1 >= 5 min blocks)
        spttrials = strcmp(stimtable.stimulus_name, 'spontaneous');
        spttrialstart = stimtable.start_time(spttrials);
        spttrialstop = stimtable.stop_time(spttrials);
        longblk_ind = find(spttrialstop - spttrialstart >= 300); % extract only the long spontaneous blocks (>= 5 min)
        ...
        Nunits = size(psthspt,ndims(psthspt)); % last dimension
        save(strcat(pathsv, 'Rspt_V1_', sesid, '.mat'), ...
            'spttrialstart', 'psthspt', 'Nunits', 'longblk_dur') % brain observatory 1.1
